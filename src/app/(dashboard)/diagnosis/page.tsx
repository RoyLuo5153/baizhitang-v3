...
const PROCESS_ATTRIBUTIONS: Record<string, Omit<AttributionEntry, 'currentValue' | 'planId' | 'planContent' | 'alreadyPushed'>> = {
  ...
  rolePlay: {
    ...
    planName: '场景实战演练',
    planHours: 5,
    planSteps: ['高频场景逐一演练', '顾虑消除专项训练', '录音复盘与改进'],
  },
  attendance: {
...
const RESULT_ATTRIBUTIONS: Record<string, Omit<AttributionEntry, 'currentValue' | 'planId' | 'planContent' | 'alreadyPushed'>> = {
  wechatAddRate: {
    ...
    threshold: 90,
    problemLabel: '加V率不达标',
    problemDesc: '首通电话安全感建立不充分，需加强治疗动机挖掘和用药反应告知',
    planName: '首通安全感强化',
    planHours: 3,
    planSteps: ['治疗动机深度挖掘训练', '用药反应与血糖波动告知演练', '价格预期与家属确认场景模拟'],
  },
  consultationRate: {
    ...
    threshold: 85,
    problemLabel: '面诊转化不足',
    problemDesc: '面诊邀约技巧欠缺，需提升顾虑消除和效果对比确认能力',
    planName: '面诊邀约特训',
    planHours: 4,
    planSteps: ['复诊预约与效果对比训练', '顾虑提前了解与消除演练', '接诊电话沟通技巧强化'],
  },
  receptionRate: {
    ...
    threshold: 80,
    problemLabel: '接诊率偏低',
    problemDesc: '面诊顾虑化解能力不足，需加强接诊电话与顾虑消除技巧',
    planName: '面诊转化突破',
    planHours: 5,
  ...
  deliveryRate: {
    ...
    planName: '签收转化突破',
    planHours: 6,
    planSteps: ['价格预期管理前置训练', '顾虑针对性消除演练', '效果对比确认与价值呈现'],
  },
  medicationRate: {
    ...
    threshold: 70,
    problemLabel: '用药率不达标',
    problemDesc: '用药反应告知与依从性引导能力不足，需加强安全感建设',
    planName: '用药沟通专项',
    planHours: 4,

...
- **四阶段(L0.5已迁移)**: 阶段1首通电话→2三天回访→3五天预约→4面诊当天。4节点对应19核心动作，每节点独立闯关
- **C-lite前端(L6已完成)**: 4节点导航+双区布局(学习区闯关+执行区动作评分)、5级评分组件(ActionScoreInput, 3视角切换)、信任度仪表盘(TrustDashboard, 4维度+总分+卡点+建议)、19动作速查(ActionQuickRef, 全局入口+搜索+4节点分组)、qc-flow嵌入TrustDashboard+3视角
- **阶段转换引擎(L1已完成)**: stage-engine.ts消费stage_rules表，自动检查闯关进度+业务指标阈值，满足条件自动UPDATE users.stage + INSERT stage_transitions记录 + 通知相关人。3条规则：1→2(闯关全通过)、2→3(闯关全通过)、3→4(闯关+面诊率50%)。幂等设计：已转换阶段不会重复触发
...
- **四阶段成长模型(L0.5已迁移)**: 首通电话(1周6关) → 三天回访(4关) → 五天预约(6关) → 面诊当天(5关)。21关按6/4/6/5分配
- **19核心动作4节点(v2迁移完成)**: 
...
- **诊断对象过滤(fixed)**: /api/diagnosis和/api/empower/alerts查询增加role_id=1过滤，确保只对trainee做诊断
...
│   │   ├── quadrant-engine.ts # 四象限分类引擎（L3）
│   │   ├── graduation-engine.ts # 出师判定引擎（L5）
│   │   ├── learning-path-engine.ts # 学习路径引擎（M1）
│   │   ├── workday-utils.ts # 工作日计算工具（M1）
